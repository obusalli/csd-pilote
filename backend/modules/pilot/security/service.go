...
func (s *Service) CreateProfile(ctx context.Context, token string, tenantID, userID uuid.UUID, input *FirewallProfileInput) (*FirewallProfile, error) {
	...
	if len(input.RuleIDs) > 0 {
		...
		if len(ruleIDs) > 0 {
			s.repo.AddRulesToProfile(profile.ID, ruleIDs)
		}
	}
...
func (s *Service) UpdateProfile(ctx context.Context, token string, tenantID, id uuid.UUID, input *FirewallProfileInput) (*FirewallProfile, error) {
	...
	if input.RuleIDs != nil {
		...
		s.repo.SetProfileRules(profile.ID, ruleIDs)
	}
...
func (s *Service) AddRulesToProfile(ctx context.Context, tenantID, profileID uuid.UUID, ruleIDs []uuid.UUID) error {
	...
	return s.repo.AddRulesToProfile(profileID, ruleIDs)
}
...
func (s *Service) ApplyTemplateToProfile(ctx context.Context, token string, tenantID, userID, templateID, profileID uuid.UUID) error {
	...
	if len(ruleIDs) > 0 {
		if err := s.repo.AddRulesToProfile(profile.ID, ruleIDs); err != nil {
...
func (s *Service) generateNftablesConfigForProfile(profile *FirewallProfile) string {
	...
	config += "# Generated by CSD-Pilote Security Module\n"
	config += fmt.Sprintf("# Profile: %s\n", profile.Name)
	config += fmt.Sprintf("# Generated at: %s\n\n", time.Now().Format(time.RFC3339))
	...
	// Filter table
	config += fmt.Sprintf("table %s filter {\n", family)
	...
	for _, chain := range chains {
		...
		config += fmt.Sprintf("    chain %s {\n", chain.nftName)
		config += fmt.Sprintf("        type filter hook %s priority 0; policy %s;\n\n", chain.hookType, policy)

		// Add base rules based on profile settings
		if chain.name == RuleChainInput {
			// Loopback rule
			if profile.AllowLoopback {
				config += "        # Allow loopback traffic\n"
				config += "        iif lo accept\n\n"
			}

			// Connection tracking
			if profile.AllowEstablished {
				config += "        # Allow established and related connections\n"
			...
			if profile.AllowICMPPing {
				config += "        # Allow ICMP ping\n"
				if family == "inet" {
					config += "        ip protocol icmp icmp type echo-request accept\n"
					config += "        ip6 nexthdr icmpv6 icmpv6 type echo-request accept\n\n"
				} else {
					config += "        ip protocol icmp icmp type echo-request accept\n\n"
				}
			}
	...
	if profile.EnableNAT {
		config += fmt.Sprintf("table %s nat {\n", family)
